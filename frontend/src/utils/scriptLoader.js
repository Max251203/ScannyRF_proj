...
// CKEditor 4.22.1 (оставим как было)
...
// ВАЖНО: pdf.js — ставим workerSrc до самого первого getDocument
export async function ensurePDFJS(){
  ...
  const wrk=[`https://cdn.jsdelivr.net/npm/pdfjs-dist@${ver}/build/pdf.worker.min.js`,`https://cdnjs.cloudflare.com/ajax/libs/pdf.js/${ver}/pdf.worker.min.js`];
  ...
  for(let i=0;i<lib.length;i++){
    try{
      await loadScript(lib[i]);
      const ok=await waitFor(()=>!!window.pdfjsLib,5000,50);
      if(!ok) continue;
      await loadScript(wrk[i]);
      try{ window.pdfjsLib.GlobalWorkerOptions.workerSrc = wrk[i]; }catch{}
      return;
    }catch(e){err=e}
  }
